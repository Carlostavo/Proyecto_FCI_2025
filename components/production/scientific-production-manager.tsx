    ...
    <div className="min-h-full bg-[#f4f8fc] pb-10">
      <section className="border-b-4 border-[#00a6d6] bg-[#00529b] px-6 py-7 text-white">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-sm font-medium text-cyan-100">Gestion academica</p><h2 className="mt-1 text-2xl font-semibold">Produccion Cientifica</h2><p className="mt-2 text-sm text-blue-100">Publicaciones, responsables, evidencias y avance sincronizado con el dashboard.</p></div>
      ...
      <div className="space-y-5 px-4 pt-6 sm:px-6">
        ...
        {investigators.length === 0 ? <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">No hay usuarios con rol investigadora. Asigna el rol desde Configuracion para registrar responsables.</div> : null}
      ...
      <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{editing ? "Editar producto cientifico" : "Registrar publicacion"}</DialogTitle><DialogDescription>Los estados alimentan automaticamente Produccion por Investigador.</DialogDescription></DialogHeader><form action={submit} className="space-y-4"><input type="hidden" name="id" value={editing?.id ?? ""} /><label className="block space-y-1.5 text-sm font-medium"><span>Titulo de la investigacion / articulo</span><Input name="titulo" value={title} onChange={(event) => setTitle(event.target.value)} required /></label><label className="block space-y-1.5 text-sm font-medium"><span>Descripcion</span><Textarea name="descripcion" value={description} onChange={(event) => setDescription(event.target.value)} /></label><label className="block space-y-1.5 text-sm font-medium"><span>Tipo de producto</span><select name="tipo" value={type} onChange={(event) => setType(event.target.value as ScientificProduct["tipo"])} className="h-8 w-full rounded-lg border border-input bg-background px-2.5"><option value="articulo_scopus">Articulo indexado Scopus</option><option value="articulo_latindex">Articulo indexado Latindex</option><option value="ponencia">Ponencia en congreso</option><option value="capitulo_libro">Capitulo de libro</option><option value="politica_publica">Insumo para politica publica</option></select></label><div className="space-y-2"><p className="text-sm font-medium">Investigadores responsables</p>{investigators.map((investigator) => <label key={investigator.id} className="flex items-center gap-2 rounded-md border border-border px-3 py-2 text-sm"><input type="checkbox" name="investigadores" value={investigator.id} checked={responsibles.includes(investigator.id)} onChange={(event) => setResponsibles((current) => event.target.checked ? [...current, investigator.id] : current.filter((id) => id !== investigator.id))} />{investigator.nombre}</label>)}</div><div className="space-y-2"><p className="text-sm font-medium">Estado del proceso</p><div className="grid grid-cols-2 gap-2">{Object.entries(STATE_LABELS).map(([value, label]) => <button key={value} type="button" onClick={() => setStatus(value as ScientificProduct["estado"])} className={cn("rounded-md border px-3 py-2 text-sm", status === value ? "border-[#0077b6] bg-[#eaf4fb] text-[#00529b]" : "border-border")}>{label}</button>)}</div><input type="hidden" name="estado" value={status} /></div><label className="block space-y-1.5 text-sm font-medium"><span>Archivo PDF / Enlace DOI</span><Input name="evidencia_url" type="url" value={evidence} onChange={(event) => setEvidence(event.target.value)} placeholder="https://doi.org/... o enlace al PDF" /></label>{message ? <p className="text-sm text-destructive">{message}</p> : null}<DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancelar</Button><Button type="submit" disabled={pending || !investigators.length}>{pending ? "Guardando..." : "Guardar publicacion"}</Button></DialogFooter></form></DialogContent></Dialog>
    </div>
  )
}
